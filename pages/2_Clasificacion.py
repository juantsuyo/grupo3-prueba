...
st.set_page_config(page_title="Clasificacion", page_icon=":wrench:")
...
edad_mujer = st.sidebar.number_input("Edad de la mujer", min_value=15, max_value=49, value=30, step=1)
edad_primera_union_iqr = st.sidebar.number_input("Edad de la primera unión", min_value=10, max_value=40, value=20, step=1)
edad_primera_relacion_iqr = st.sidebar.number_input("Edad de la primera relación sexual", min_value=10, max_value=40, value=18, step=1)
edad_conyuge_iqr = st.sidebar.number_input("Edad del cónyuge", min_value=15, max_value=70, value=35, step=1)

st.sidebar.markdown("### Frecuencia de medios")
frec_periodico = st.sidebar.selectbox(
...
frec_radio = st.sidebar.selectbox(
    "Frecuencia de radio",
    options=[0, 1, 2, 3],
    format_func=lambda x: {
        0: "Nunca",
...
quintil_riqueza = st.sidebar.selectbox(
    "Quintil de riqueza",
    options=[1, 2, 3, 4, 5],
    format_func=lambda x: {
        1: "Más pobre",
...
educ_mujer_rec = st.sidebar.selectbox(
...
educ_conyuge_rec = st.sidebar.selectbox(
...
alcohol_conyuge = st.sidebar.selectbox(
...
control_machista = st.sidebar.slider("Control machista (escala)", min_value=0, max_value=6, value=1, step=1)
